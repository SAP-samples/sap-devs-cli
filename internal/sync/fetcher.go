...
func FetchArchive(rawURL, destDir, token string) error {
	...
	if resp.Request != nil && resp.Request.URL != nil && resp.Request.URL.Hostname() == parsedURL.Hostname() && strings.Contains(resp.Request.URL.Path, "/login") {
		return fmt.Errorf("authentication required for %s — set GH_TOKEN or run 'sap-devs config token'", parsedURL.Host)
	}

	zr, err := zip.NewReader(bytes.NewReader(data), int64(len(data)))
	if err != nil {
		if resp.StatusCode == http.StatusOK {
			return fmt.Errorf("open zip: %w — server returned non-zip content; if %s requires authentication, set GH_TOKEN or run 'sap-devs config token'", err, parsedURL.Host)
		}

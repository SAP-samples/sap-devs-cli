...
Windows `.exe` files are Authenticode-signed via [SignPath.io](https://signpath.io) (free for OSS). Signing runs as a post-release step in `.github/workflows/sign-windows.yml`, triggered automatically after the "Release Tray Binary" workflow completes successfully.
